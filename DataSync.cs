                ...
                foreach (SpiraSoapService.RemoteDataMapping projectMapping in projectMappings)
                {
                    ...
                    foreach (SpiraSoapService.RemoteIncident remoteIncident in incidentList)
                    {
                        try
                        {
                            ...
                            {
                                //We can't connect so go to next project
                                LogErrorEvent("Unable to connect to Custom JIRA project " + customJiraProject + ", please check that the JIRA login '" + externalLogin + "' has the permissions to access this project", EventLogEntryType.Error);
                                continue;
                            }
                            else
