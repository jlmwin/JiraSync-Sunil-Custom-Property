            ...
            try
            {
                ...
                foreach (SpiraSoapService.RemoteDataMapping projectMapping in projectMappings)
                {
                    ...
                    foreach (SpiraSoapService.RemoteIncident remoteIncident in incidentList)
                    {
                        try
                        {
                            //Get Custom Jira Project
                            int valueId = remoteIncident.CustomProperties.Where(i => i.Definition.Name.Equals("JIRA Project ID")).FirstOrDefault().IntegerValue.GetValueOrDefault();
                            string jiraProjectID = remoteIncident.CustomProperties.Where(i => i.Definition.Name.Equals("JIRA Project ID")).FirstOrDefault().Definition.CustomList.Values.Where(c => c.CustomPropertyValueId.Equals(valueId)).FirstOrDefault().Name;
                            JiraProject customJiraProject = jiraProjects.FirstOrDefault(j => j.Id.Equals(jiraProjectID));

                            LogErrorEvent("Incident: " + remoteIncident.IncidentId + "ValueId: " + valueId + "JIRA Project ID: " + jiraProjectID + "Custom Jira Project Name: " + customJiraProject.Name);
                            ...
                            else
                            {
                                ...
                                ProcessIncident(projectId, spiraImportExport, remoteIncident, newIncidentMappings, newReleaseMappings, oldReleaseMappings, incidentCustomPropertyMappingList, incidentCustomPropertyValueMappingList, incidentCustomProperties, incidentMappings, customJiraProject, jiraManager, productName, incidentSeverityMappings, incidentPriorityMappings, incidentStatusMappings, incidentTypeMappings, userMappings, releaseMappings, incidentComponentMappings);
                            }
                        }
                        catch (Exception exception)
                        {
